...
p70_capital_need_immobile(t,h,kli) = sum(supreg(h,i),
                                      (i70_cost_regr(i,kli,"cost_regr_a") + i70_cost_regr(i,kli,"cost_regr_b")  
...
*' Estimate capital stock based on capital remuneration. We assume that in 1994 and 1995 production is the same and the stocks gets depreciated from 1994.
  p70_capital_immobile(t,i,kli) =  sum(supreg(h,i), p70_capital_need_immobile(t,h,kli) ) * p70_prod_init(i,kli)*(1-s70_depreciation_rate);

else

*' Update of existing stocks

  p70_capital_immobile(t,i,kli)=p70_capital_immobile(t,i,kli)*(1-s70_depreciation_rate)**(m_timestep_length);
